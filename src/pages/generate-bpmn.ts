...
const openai = new OpenAI({
  apiKey: process.env.OPENAI_API_KEY, // Guarde sua chave em variáveis de ambiente
});
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'POST') {
    ...
    try {
      // Prompt específico para BPMN XML, incluindo a necessidade de collaboration, participants e lanes
      // como visto na estrutura do seu commit [1]
      const systemPrompt = `
      ...
      const completion = await openai.chat.completions.create({
        ...
        temperature: 0.5, // Um valor mais baixo para saídas mais consistentes e estruturadas como XML
      });
